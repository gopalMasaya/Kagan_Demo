...
console.log(err);
 ...
 age =   new Input  ("גיל:",4, width - 400,330,200,25);
 gender = new Input ("מין",5,  width - 400,390,200,25);

 circul =  new Input    ("מעגל שיוך" ,7,width - 830,150,200,25);
 contactmen =  new Input("איש קשר"   ,8,width - 830,210,200,25);
 needAride =   new Input("להסעה"     ,9,width - 830,270,200,25);
...
if(keyIsDown(BACKSPACE)){
      ...
      if( messagelength > 0){
       message[mIndex] =  message[mIndex].substring(0,messagelength-messagelength); }
      if(count > 0){ count-=1;}
       return;
}
}
...
console.log(Password+"  "+limit);
  ...
  if (key == '\n' ) {  count = 0;recive = false;} else {
    ...
    count++;}
     ...
     if (keyCode == BACKSPACE)  {
       fill(100,0,0); rect(100,100,50,50);
           if( messagelength > -1){
            message[mIndex] =  message[mIndex].substring(0,messagelength-1);
           }
           if(count > 0){ count-=1;}
